...
connectomeToggleBtn.addEventListener('click', function () {
	...
	connectomeToggleBtn.textContent = 'Dots';
});
...
document.addEventListener('visibilitychange', function () {
	if (document.hidden) {
		...
		BRAIN.stimulate.foodSmelled = false;
		BRAIN.stimulate.foodVisible = false;
...
function handleCanvasMousedown(event) {
	...
	if (activeTool === 'feed' || activeTool === 'flower') {
		...
		food.push({ x: cx, y: cy, radius: 10, feedStart: 0, feedDuration: 0, eaten: 0,
			kind: activeTool === 'flower' ? 'flower' : 'food' });
...
function applyTouchTool(cx, cy) {
	if (BRAIN.childPathwayDisabled && BRAIN.childPathwayDisabled.touch) return;
...
function updateBehaviorState() {
	var now = Date.now();
	var pathways = BRAIN.childPathwayDisabled || {};
	var mustStopCurrentBehavior = pathways.movement ||
		(pathways.taste && behavior.current === 'feed') ||
		(pathways.touch && (behavior.current === 'startle' || behavior.current === 'brace' || behavior.current === 'groom'));
	if (mustStopCurrentBehavior && behavior.current !== 'idle') {
		behavior.current = 'idle';
...
function drawFood() {
	...
	for (var i = 0; i < food.length; i++) {
		...
		if (f.kind === 'flower') {
			...
			ctx.strokeStyle = '#4f9b54';
			ctx.lineWidth = 3;
			ctx.beginPath();
			ctx.moveTo(0, f.radius * 0.3);
			ctx.lineTo(0, f.radius * 2.2);
			ctx.stroke();
			ctx.fillStyle = '#f472b6';
			for (var petal = 0; petal < 6; petal++) {
				var a = petal * Math.PI / 3;
				ctx.beginPath();
				ctx.arc(Math.cos(a) * f.radius * 0.75, Math.sin(a) * f.radius * 0.75, f.radius * 0.52, 0, Math.PI * 2);
				ctx.fill();
			}
			ctx.beginPath();
			ctx.arc(0, 0, f.radius * 0.48, 0, Math.PI * 2);
...
function update(dt) {
	...
	var movementDisabled = BRAIN.childMovementDisabled === true;
	if (movementDisabled) {
		/* Classroom "movement off" is an output gate. The neural model and
		 * internal drives keep running, but no hand-authored movement rule may
		 * move or rotate the visible body around the muted motor pathway. */
		speed = 0;
	...
	if (!movementDisabled && (edgeBias !== 0 || edgeBiasY !== 0)) {
	...
	if (!movementDisabled && fly.x < 0) {
		fly.x = 0;
		BRAIN.stimulate.touch = true;
		touchResetTime = Math.max(touchResetTime, Date.now() + 2000);
	} else if (!movementDisabled && fly.x > window.innerWidth) {
		fly.x = window.innerWidth;
		BRAIN.stimulate.touch = true;
		touchResetTime = Math.max(touchResetTime, Date.now() + 2000);
	}
	if (!movementDisabled && fly.y < 44) {
		fly.y = 44;
		BRAIN.stimulate.touch = true;
		touchResetTime = Math.max(touchResetTime, Date.now() + 2000);
	} else if (!movementDisabled && fly.y > window.innerHeight) {
		fly.y = window.innerHeight;
		BRAIN.stimulate.touch = true;
		touchResetTime = Math.max(touchResetTime, Date.now() + 2000);
	}

	// Food is a multi-sense activity: smell finds it at a distance, vision can
	// spot a visible flower, and taste is the final check before feeding.
	var pathways = BRAIN.childPathwayDisabled || {};
	var smellAvailable = !pathways.smell;
	var visionAvailable = !pathways.vision;
	var tasteAvailable = !pathways.taste;
	BRAIN.stimulate.foodContact = false;
	BRAIN.stimulate.foodNearby = false;
	BRAIN.stimulate.foodSmelled = false;
	BRAIN.stimulate.foodVisible = false;
	for (var i = 0; i < food.length; i++) {
		...
		var smelledFood = smellAvailable && dist <= 150;
		var sawFood = visionAvailable && dist <= 260;
		var tastedFood = tasteAvailable && dist <= 20;
		if (smelledFood || sawFood || tastedFood) {
			BRAIN.stimulate.foodNearby = true;
			if (smelledFood) BRAIN.stimulate.foodSmelled = true;
			if (sawFood) BRAIN.stimulate.foodVisible = true;
			if (tastedFood) {
...
function draw() {
	...
	if (ll >= 1) {
		canvas.style.backgroundColor = '#bfe3a1';
	} else if (ll >= 0.5) {
		canvas.style.backgroundColor = '#a9ce87';
	} else {
		canvas.style.backgroundColor = '#769d61';
	}
	...
	if (typeof drawChildGarden === 'function') drawChildGarden(ctx);
	drawFood();
	drawRipples();
	drawWindArrow();
