...
function evaluateBehaviorEntry() {
	...
	var touchAirAvailable = !pathways.touch;

	// The classroom movement switch is an output lesion: internal drives can
	// still change, but they cannot select a visible motor behaviour.
	if (pathways.movement) return 'idle';

	if (touchAirAvailable && BRAIN.accumStartle > BEHAVIOR_THRESHOLDS.startle && !isCoolingDown('startle', now)) {
		return 'startle';
	}
	...
	if (tasteAvailable && feedReady && hasNearbyFood() && !isCoolingDown('feed', now)) {
		return 'feed';
	}
	if (BRAIN.accumGroom > BEHAVIOR_THRESHOLDS.groom && !isCoolingDown('groom', now)) {
		return 'groom';
	}
	if (touchAirAvailable && BRAIN.stimulate.wind && BRAIN.stimulate.windStrength < 0.5 &&
	...
	var visionAvailable = !BRAIN.childPathwayDisabled || !BRAIN.childPathwayDisabled.vision;
	if (visionAvailable && BRAIN.stimulate.lightLevel > BEHAVIOR_THRESHOLDS.phototaxisLight &&
