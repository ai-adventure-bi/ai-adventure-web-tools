...
var groupMuted = null;            // Uint8Array[numGroups], set by classroom experiment controls
...
function buildGroupStructures() {
	...
	groupMuted = new Uint8Array(numGroups);
}
...
function tick() {
	...
	for (var g = 0; g < numGroups; g++) {
		if (!groupActive[g] || groupMuted[g]) continue;
	...
	if (sustainedIndices) {
		for (var k = 0; k < sustainedIndices.length; k++) {
			var si = sustainedIndices[k];
			if (si < N && !groupMuted[groupId[si]] && refractory[si] === 0) {
	...
	for (var g = 0; g < numGroups; g++) {
		if (!groupMuted[g] && groupRecvInput[g] && !groupActive[g]) {
			groupActive[g] = 1;
			groupCooldown[g] = COOLDOWN_TICKS;
		}
	}
...
self.onmessage = function (e) {
	...
	case 'stimulate':
		...
		for (var k = 0; k < indices.length; k++) {
			var idx = indices[k];
			if (idx < N && (!groupMuted || !groupMuted[groupId[idx]])) {
				V[idx] += intensities[k];
	...
	case 'setGroupsMuted':
		if (!groupMuted || !groupOffset) break;
		var ids = e.data.groupIds || [];
		for (var mi = 0; mi < ids.length; mi++) {
			var mutedId = ids[mi];
			if (mutedId < 0 || mutedId >= groupMuted.length) continue;
			groupMuted[mutedId] = e.data.muted ? 1 : 0;
			if (e.data.muted) {
				groupActive[mutedId] = 0;
				groupCooldown[mutedId] = 0;
				for (var mn = groupOffset[mutedId]; mn < groupOffset[mutedId + 1]; mn++) {
					V[mn] = 0; fired[mn] = 0; refractory[mn] = 0;
				}
			}
		}
		break;
