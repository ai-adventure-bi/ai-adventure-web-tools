...
(function () {
	...
	var mutedGroupIds = {};      // group IDs disabled by the classroom experiment controls
	...
	function collectStimulationSegments() {
		...
		if (BRAIN.stimulate.foodSmelled) {
	...
	/* Public classroom control: fully mute named functional groups in the
	 * full worker simulation. The worker blocks stimulation, incoming signals,
	 * outgoing signals, and clears current activity for those groups. */
	BRAIN.setMutedGroups = function (names, muted) {
		if (!workerReady || !worker) return false;
		var ids = [];
		for (var i = 0; i < names.length; i++) {
			var gid = groupNameToId[names[i]];
			if (gid === undefined) continue;
			if (muted) mutedGroupIds[gid] = true;
			else delete mutedGroupIds[gid];
			ids.push(gid);
		}
		worker.postMessage({type: 'setGroupsMuted', groupIds: ids, muted: muted});
		return true;
	};
